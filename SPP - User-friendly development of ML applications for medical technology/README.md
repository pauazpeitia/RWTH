...
```bash
git clone [https://git-ce.rwth-aachen.de/wzl-mq-ms-rpc/code/research/predictive-quality/vm-pipeline-gui](https://git-ce.rwth-aachen.de/wzl-mq-ms-rpc/code/research/predictive-quality/vm-pipeline-gui)
cd vm-pipeline-gui
2. Environment Configuration Create a .env file in the root directory (use .env.example as a template):
...
## Team & Contributions
Lab for Machine Tools and Production Engineering (WZL). Chair of Intelligence in Quality Sensing.
...
Mauricio Wilneder - mauricio.wilneder@rwth-aachen.de
    ...
    Frontend Architecture: UI Layout, Drag-and-Drop Implementation, Parameter Panel Design, S3 Config UI.
...
Pau Azpeitia - pau.azpeitia.bergos@rwth-aachen.de
    ...
    Backend Architecture: Django API Setup, Argo REST API Integration, YAML Download Endpoint + Send to Argo Endpoint, Management of all Frontend–Backend communication.

    Frontend Architecture: Sidebar, Parameter Panel Logic (Entrypoints, Parameters, Schnittstelle), Buttons.

    Workflow Engine: Hera Integration, S3 Secret Implementation, Path Management.

    DevOps: Dockerfile Creation (Multi-Stage), UX Improvements.

    Data: Dataset Structure & Analysis,  Understand the existing Argo methods.

    Testing: Validation of Test Runs.

Jean - jean.alsino@rwth-aachen.de

    Kubernetes Manifests, Sphinx Documentation, Filter Functions in Sidebar.

    Data: Understand the existing Argo methods.

Monique - monique.zentgraf@rwth-aachen.de

    Sphinx Documentation.

Repository Access
    Repository URL: https://git-ce.rwth-aachen.de/wzl-mq-ms-rpc/code/research/predictive-quality/vm-pipeline-gui

Notes:  Access requires connection via RWTH VPN or Eduroam.
